...
public class Fuente {
	...
	/**
	 * Crea una nueva instancia Fuente.
	 * Inicializa Arreglo(Abecedario) en el cual se inicializan todos los ASCIICharacter
	 * ordenados de forma ascendente siguiendo la enumeración ASCII y luego crea
	 * la cadena de contendra el arte ASCII
	 * @param texto Texto que queremos convertir.
	 */
	public Fuente(String texto) {
		...
		//Buscando todos los caracteres del texto y retornado su version ASCII
	...
	/**
	 * Busca un caraster el el abecedario.
	 * @param caracter Caraster que deseamos buscar
	 * @return retorna el texto artASCII del carater si no lo encuentra retornara el ASCIICharacter espacio( )
	 */
	...
	/**
	 * Busca y une todos los carateres para formar un solo texto en horisontal aparte de crear
	 * las uniones correspondiente entre lineas.
	 * @param Texto Cadena, con el contenido que deseamos utilizar
	 * @return retorna una cadena con la ArtASCII
	 */
	private String CreacionDelTexto(String Texto) {
		...
		for (int Linea = 0; Linea < 8; Linea++) {
			...
			for (int Letras = 1; Letras < this.AsciiTexto.length; Letras++) {
				...
				// Verifica todos los casos de Unio.
				if (last == first) {
					union = first;
				} else if (last == '░') {
					union = first;
		...
		// combiente en lineas los espacios necesario de lo contrario aplica un salto de linea.
